...
export class ProjectsController {
  ...
  @ApiStandardResponse(ProjectResponseDto, 'Project created successfully', HttpStatus.CREATED)
  @ApiCommonErrors({ badRequest: true, unauthorized: true })
  @UseGuards(JwtAuthGuard)
  createProject(@GetUser('userid') ownerId: string, @Body() createProjectDto: CreateProjectDto): Promise<ProjectResponseDto> {
    return this.projectsService.create(createProjectDto, +ownerId);
  }

  @ApiOperation({ summary: 'Retrieve all projects', description: 'Available only in non-production environments for testing or seeding.'})
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Retrieve a project by ID', description: 'Requires be part of the project.' })
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Update a project', description: 'Requires owner or admin access to modify project data.' })
  ...
  @Patch(':projectId')
  updateProject(@Param('projectId') projectId: string, @Body() updateProjectDto: UpdateProjectDto): Promise<ProjectResponseDto> {
    return this.projectsService.update(+projectId, updateProjectDto);
  }

  @ApiBearerAuth()
  @ApiOperation({ summary: 'Delete a project', description: 'Requires project owner privileges.' })
  ...
  async createTask(@Param('projectId') projectId: string, @Body() createTaskDto: CreateTaskDto): Promise<TaskResponseDto> {
    return this.tasksService.create(+projectId, createTaskDto);
  }

  @ApiBearerAuth()
  @ApiOperation({ summary: 'Retrieve a task by ID', description: 'Requires be part of the project.' })
  @ApiStandardResponse(TaskResponseDto, 'Task found')
  @ApiCommonErrors({ unauthorized: true, notFound: true })
  @UseGuards(JwtAuthGuard, ProjectMemberGuard)
  @Get(':projectId/tasks/:taskId')
  findOneTask(@Param('projectId') projectId: string, @Param('taskId') taskId: string): Promise<TaskResponseDto> {
    return this.tasksService.findOne(+projectId, +taskId);
  }

  @ApiBearerAuth()
  @ApiOperation({ summary: 'Retrieve all project task ', description: 'Requires be part of the project.' })
  @ApiStandardResponse(TaskResponseDto, 'Tasks found')
  @ApiCommonErrors({ unauthorized: true, notFound: true })
  @UseGuards(JwtAuthGuard, ProjectMemberGuard)
  @Get(':projectId/tasks')
  async getProjectTasks(@Param('projectId') projectId: string): Promise<TaskResponseDto[]> {
    return this.tasksService.findAllByProject(+projectId);
  }

  @ApiBearerAuth()
  @ApiOperation({ summary: 'Update a task', description: 'Requires owner or admin access to modify task data.' })
  @ApiStandardResponse(TaskResponseDto, 'Task updated successfully')
  @ApiCommonErrors({ badRequest: true, unauthorized: true, notFound: true })
  @UseGuards(JwtAuthGuard, ProjectAdminGuard)
  @Patch(':projectId/tasks/:taskId')
  updateTask(@Param('projectId') projectId: string, @Param('taskId') taskId: string, @Body() updateTaskDto: UpdateTaskDto): Promise<TaskResponseDto> {
    return this.tasksService.update(+projectId, +taskId, updateTaskDto );
  }

  @ApiBearerAuth()
  @ApiOperation({ summary: 'Delete a task', description: 'Requires project admin privileges.' })
