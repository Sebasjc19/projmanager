...
export class TasksService {
   ...
   */
  async create(projectId: number, createTaskDto: CreateTaskDto): Promise<TaskResponseDto> {
    ...
    if (createTaskDto.assignedUserIds?.length) {
      const users = await this.usersService.findByIds(createTaskDto.assignedUserIds);
    ...
    if (!taskWithRelations) {
      throw new NotFoundException(`Task with ID ${savedTask.id} not found after creation`);
    }
   ...
   */
  async update(projectId: number, taskId: number, updateTaskDto: UpdateTaskDto): Promise<TaskResponseDto> {
    const task = await this.taskRepository.findOne({
      where: { id: taskId, project: { id: projectId } },
      relations: ['assignedUsers', 'project']
    });
    ...
    if (updateTaskDto.assignedUserIds) {
      const users = await this.usersService.findByIds(updateTaskDto.assignedUserIds);
      task.assignedUsers = users;
    }
